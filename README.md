...
[![MS Teams Deploy Card](https://github.com/uccodingducks/ms-teams-deploy-card/actions/workflows/main.yml/badge.svg)](https://github.com/uccodingducks/ms-teams-deploy-card/actions/workflows/main.yml)
...
jobs:
  build:
    ...
    steps:
      ...
      - uses: uccodingducks/ms-teams-deploy-card@v1 #  or "./" if in a local set-up
        if: always()
        with:
...
```yaml
- uses: uccodingducks/ms-teams-deploy-card@v1
  with:
    github-token: ${{ github.token }}
    webhook-uri: ${{ secrets.MS_TEAMS_WEBHOOK_URI }}
    custom-facts: |
